...
Java_com_putri_esp_Overlay_DrawOn(JNIEnv *env, jclass , jobject espView, jobject canvas) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_putri_esp_Overlay_Close(JNIEnv *,  jobject ) {
   Close();
}
extern "C" JNIEXPORT void JNICALL
Java_com_putri_esp_FloatLogo_SettingValue(JNIEnv *,  jobject ,jint code,jboolean jboolean1) {
    switch((int)code){
        case 1:
...
Java_com_putri_esp_Overlay_getReady(JNIEnv *, jclass ,int typeofgame) {
    int sockCheck=1;
    if (!Create()) {
